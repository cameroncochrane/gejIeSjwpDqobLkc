"""
train_sagemaker_tl.py

Single-file TensorFlow + Amazon SageMaker training workflow for transfer learning (TL) models.

LOCAL EXECUTION
---------------
Run from Windows:

    python train_sagemaker_tl.py

Run from Mac:

    python3 train_sagemaker_tl.py
...
Run as this when you don't want to re-upload data:
    ...
    python train_sagemaker_tl.py --skip-data-upload
...
MODEL_NAME = "vgg16_ff_da_tl_model"
...
DATA_FILE = "processed_data_vgg16.pkl"
...
def build_model(input_shape):
    ...
    base_tl = tf.keras.applications.VGG16(weights="imagenet", include_top=False, input_shape=input_shape)
    base_tl.trainable = False
    
    model = Sequential([
        # Call base TL model (defined above):
        base_tl,

        # DA (if necessary):
        data_augmentation,

        # Rest of model i.e. classifier head:
    ...
    # Keep the same model compilation parameters among all TL model training runs for consistency:
